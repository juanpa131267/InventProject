...
@section('title', 'Rentalo - Editar Publicación')
...
<div class="container py-4">
    <div class="text-center mb-4">
        ...
        <h2 class="mb-0 mt-2">Editar Publicación</h2>
    </div>
            ...
            <form action="{{ route('inmueble.update', $inmueble->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <div class="form-group mb-3">
                            ...
                            <textarea name="descripcion" id="descripcion" class="form-control" rows="3" required>{{ old('descripcion', $inmueble->descripcion) }}</textarea>
                            </div>

                        <div class="form-group mb-3">
                            <label for="detalles_habitacion" class="form-label">Detalles de la Habitación <span class="text-danger">*</span></label>
                            <textarea name="detalles_habitacion" id="detalles_habitacion" class="form-control" rows="2" required>{{ old('detalles_habitacion', $inmueble->detalles_habitacion) }}</textarea>
                            </div>

                            <div class="form-group mb-3">
                                <label for="estado_disponibilidad" class="form-label">
                                    Estado de Disponibilidad <span class="text-danger">*</span>
                                </label>
                                <select name="estado_disponibilidad" id="estado_disponibilidad" class="form-control" required>
                                    <option value="" disabled selected>Seleccione el estado de disponibilidad</option>
                                    <option value="disponible" {{ old('estado_disponibilidad', $inmueble->estado_disponibilidad) == 'disponible' ? 'selected' : '' }}>
                                        Disponible
                                    </option>
                                    <option value="no_disponible" {{ old('estado_disponibilidad', $inmueble->estado_disponibilidad) == 'no_disponible' ? 'selected' : '' }}>
                                        No Disponible
                                    </option>
                                </select>
                                <small id="estado_disponibilidad_help" class="form-text text-muted">
                                    Selecciona si el inmueble está disponible o no para alquiler.
                                </small>
                            </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="number" name="precio" id="precio" class="form-control" required value="{{ old('precio', $inmueble->precio) }}">
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <textarea name="direccion" id="direccion" class="form-control" rows="3" required>{{ old('direccion', $inmueble->ubicacion->direccion) }}</textarea>
                            </div>

                        <div class="form-group mb-3">
                            <label for="coordenadas" class="form-label">Coordenadas (Lat, Long) <span class="text-danger">*</span></label>
                            <input type="text" name="coordenadas" id="coordenadas" class="form-control" required value="{{ old('coordenadas', $inmueble->coordenadas) }}">
                    ...
                    <div class="card-body">
                            ...
                            @foreach($inmueble->imagenes as $index => $imagen)
                                <div class="imageField mb-3">
                                    ...
                                    <div class="form-group">
                                        <label for="descripcion_imagen_{{ $imagen->id }}" class="form-label">Descripción de la Imagen</label>
                                        <input type="text" name="descripcion_imagen[{{ $imagen->id }}]" id="descripcion_imagen_{{ $imagen->id }}" class="form-control" value="{{ old('descripcion_imagen.' . $imagen->id, $imagen->descripcion) }}">
                                    </div>

                                    <div class="text-center mb-3">
                                        <label for="Vista previa de la imagen" class="form-label">Vista previa de la imagen cargada</label>
                                        <img src="{{ asset($imagen->url_imagen) }}" alt="Imagen del inmueble" class="img-fluid shadow-lg rounded mx-auto d-block" style="max-width: 50%; height: auto;">
                                    </div>

                                    <button type="button" class="btn btn-danger cancelImageBtn">Eliminar imagen</button>
                                </div>
                            @endforeach
                        </div>
                        ...
                        <div class="imageField mb-3">
                            ...
                            <div class="form-group">
                                ...
                                <input type="file" name="imagenes[]" class="form-control" onchange="previewImage(event, this)">
                            </div>
                            <div class="form-group">
                ...
                <div class="d-flex justify-content-center">
                    <button type="submit" class="btn btn-primary mx-2">
                        <i class="bi bi-save-fill"></i> Editar
                    </button>
                    <a href="{{ url('/administrar-publicaciones') }}" class="btn btn-danger mx-2">
...
document.addEventListener('click', function (event) {
    if (event.target.classList.contains('cancelImageBtn')) {
        event.target.closest('.imageField').remove();
    }
});
